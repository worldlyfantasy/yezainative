...
- 当时仅有一个用户初始化云函数，后端能力尚未形成统一接口层。
- 当时页面主要通过 `miniprogram/services/*` 取数，而不是直接从页面读取 mock。

### 2.2 当前数据入口

Phase 0 启动时，mock 依赖主要集中在内容、订单与用户三组前端服务入口。

这些文件现已在 cloud-only 收口过程中移除，但当时确实说明了改造入口相对集中、页面层改动可控。
...
其中内容服务入口在当时已经近似一个前端 BFF，但里面混了数据源、业务规则和文案模板。后续接后端前若不先拆边界，接口会越来越乱。
